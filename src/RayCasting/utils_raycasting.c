...
/*   Updated: 2025/02/07 11:13:05 by okapshai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx->ray->wall_hit_y = (float)mlx->player[Y_PIXEL] + mlx->ray->dist
		* sin(norming_angle(mlx->ray->angle));
	if (mlx->ray->hit_vert_wall == 0)
	{
		if (ceil(mlx->ray->wall_hit_y)
			- mlx->ray->wall_hit_y <= mlx->ray->wall_hit_y
			- floor(mlx->ray->wall_hit_y))
			mlx->ray->wall_hit_y = ceil(mlx->ray->wall_hit_y);
		else
			mlx->ray->wall_hit_y = floor(mlx->ray->wall_hit_y);
	}
	mlx->ray->wall_hit_x = (float)mlx->player[X_PIXEL] + mlx->ray->dist
		* cos(norming_angle(mlx->ray->angle));
	if (mlx->ray->hit_vert_wall == 1)
	{
		if (ceil(mlx->ray->wall_hit_x)
			- mlx->ray->wall_hit_x <= mlx->ray->wall_hit_x
			- floor(mlx->ray->wall_hit_x))
...
void	put_pix_to_img(t_mlx *mlx, int x, int y, int color)
{
	...
	pix = ((char *)mlx->image[PLACEHOLDER].address + (y
				...
				* (mlx->image[PLACEHOLDER].bits_per_pixel / 8)));
	*(int *)pix = color;
	return ;
}
